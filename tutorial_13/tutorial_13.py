...
for i in range(4000):
    ...
    curr_prediction = sess.run(prediction, feed_dict={x_data: rand_x, y_target: rand_y})
    ...
    if (i + 1) % 100 == 0:
        print('Loss = ' + str(temp_loss) + ' Accuracy = ' + str(temp_acc_train))

curr_acc_test = sess.run(accuracy, feed_dict={x_data: x_vals_train, y_target: np.transpose([y_vals_train])})
print('Final Test Accuracy : ' + str(curr_acc_test))
